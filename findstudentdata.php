<?php
   ...
   $teacherid= mysqli_real_escape_string($db,$_POST['proc-id']);
   $checkuser=mysqli_query($db,"SELECT * from proctortable where tid='$teacherid' and usn='$myusername' ");
   $arr1=mysqli_fetch_assoc($checkuser);
   $checkusn=$arr1['usn'];
   if($myusername==$checkusn){
      echo "yes available";
   }
   else{
     echo "not available you are not the proctor of the student";
     die();
   }
   $ses_sql = mysqli_query($db,"SELECT usn from activcheckt where usn = '$myusername' ");
   $fetchlogin = mysqli_query($db,"SELECT * from activcheckt where usn='$myusername'");
   $fetchdata = mysqli_query($db,"SELECT * from testtable where usn='$myusername'");
   $fetchtotal = mysqli_query($db,"SELECT total from testtable where usn='$myusername'");
